...
public class Post {
    ...
    private static final String selectPostByUserQuery = "SELECT p.id, p.title, p.content, p.user, u.first_name FROM posts p INNER JOIN users u ON u.id = p.user WHERE p.user = ? ORDER BY p.id DESC";
    private static final String selectAllPostsQuery = "SELECT p.id, p.title, p.content, p.user, u.first_name FROM posts p INNER JOIN users u ON u.id = p.user ORDER BY p.id DESC";
